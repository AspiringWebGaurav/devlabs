...
export const AdminSidebar: React.FC = () => {
  ...
  const handleSignOut = async () => {
    ...
    setTimeout(() => {
      router.replace("/admin/login?signedOut=true");
      router.refresh();
    }, 400);
  };
  ...
  return (
    <>
      <aside className="w-full md:w-64 bg-[#FFFFFF] border-r border-[#E5E7EB] shrink-0 p-4 sm:p-5 flex flex-col justify-between h-full min-h-[calc(100vh-57px)]">
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.id}
                    href={item.href}
                    className={`flex items-center gap-3 px-3.5 py-2.5 rounded-sm text-xs font-mono transition-all duration-150 border ${
                      isActive
        ...
        {/* Bottom: User Profile & Sign-Out (Left Bracket Bottom) */}
        <div className="pt-4 border-t border-[#F1F5F9] space-y-3">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2 px-3 text-xs font-mono text-[#991B1B] hover:text-[#FFFFFF] bg-[#FEF2F2] hover:bg-[#DC2626] border border-[#FCA5A5] hover:border-[#DC2626] rounded-sm transition-all duration-150 cursor-pointer shadow-2xs active:scale-[0.99] disabled:opacity-60"
            title="Sign out of Admin Console"
          >
            <FaRightFromBracket className={`w-3 h-3 shrink-0 ${isSigningOut ? "animate-spin" : ""}`} />
